...
void handleReceived(uint8_t c) {
    if ((expectedResponses.isEmpty())) return;
...
void drawRect(
  ...
  uint8_t data[] = { 
    0x05, 
    mode, 
    highByte(color), lowByte(color),
    highByte(x0), lowByte(x0),
    highByte(y0), lowByte(y0), 
    highByte(x1), lowByte(x1),
    highByte(y1), lowByte(y1),
  };
  send(data, 12);
...
void setup() {
  ...
  expectResponse(ok, "handshake OK", noop);
}
